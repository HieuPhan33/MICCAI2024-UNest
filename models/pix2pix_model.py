...
class Pix2PixModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_l1', type=float, default=5.0, help='weight for L1 loss')
        ...
        parser.add_argument('--out_kernel', type=int, default=3, help='kernel size for output convolution layer')
    ...
    def __init__(self, opt):
        ...
        # self.netG_A = VisionTransformer(config, img_size=224, num_classes=21843, zero_head=False, vis=False)
        # self.netG_A.load_from(weights = np.load(config.pretrained_path))
        # self.netG_B = VisionTransformer(config, img_size=224, num_classes=21843, zero_head=False, vis=False)
        # self.netG_B.load_from(weights = np.load(config.pretrained_path))
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, 
                                        out_kernel=self.opt.out_kernel,
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.AdamW(itertools.chain(self.netG_A.parameters()), lr=opt.lr, betas=(opt.beta1, 0.9))
            self.optimizer_D = torch.optim.AdamW(itertools.chain(self.netD_A.parameters()), lr=opt.lr, betas=(opt.beta1, 0.9))
    ...
    def backward_D_A(self):
        ...
        fake_B = self.fake_A_pool.query(self.fake_B)
    ...
    def backward_G(self):
        ...
        self.loss_l1_A = self.opt.lambda_l1 * self.criterionL1(self.fake_B, self.real_B)
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A  + self.loss_idt_A + self.loss_l1_A 
