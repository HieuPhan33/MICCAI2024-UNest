...
class Compose:
    ...
    def __call__(self, img: Tensor, mask=None):
...
class CenterCrop:
    ...
    def __call__(self, img: Tensor, mask=None):
        if mask is not None:
            return TF.center_crop(img, self.size), TF.center_crop(mask, self.size)
        return TF.center_crop(img, self.size), None

class RandomPerspective(torch.nn.Module):
    def __init__(self, distortion_scale=0.5, p=0.5, interpolation=T.InterpolationMode.BILINEAR, seg_fill=0):
        super().__init__()
        self.p = p
    ...
    def forward(self, img, mask):
        ...
        if torch.rand(1) < self.p:
            ...
            return TF.perspective(img, startpoints, endpoints, self.interpolation, 0),\
             TF.perspective(mask, startpoints, endpoints, self.interpolation, self.seg_fill)
        return img, mask
...
class Pad:
    ...
    def __call__(self, img: Tensor, mask: Tensor=None) -> Tuple[Tensor, Tensor]:
        ...
        if mask is not None:
            ...
            return TF.pad(img, padding), None
